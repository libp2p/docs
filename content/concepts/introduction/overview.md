...
please don't hesitate to [reach out](../contribute/community), or help
...
- **Extensive transport configurability**: libp2p provides a set of specifications that can be adapted to
  support various [transport protocols](../transports/overview), allowing libp2p applications to operate
...
- **Security**: libp2p includes [several security features](../security/security-considerations),
  such as peer identity verification using public key cryptography and
  [encrypted communication](../secure-comm/overview) between peers using modern cryptographic algorithms.
...
- **Robustness**: libp2p is a robust and reliable networking protocol that is designed to withstand stress,
  ...
  [mitigation techniques](../security/dos-mitigation).
...
- **Resiliency**: P2P networks are often more resilient than traditional client-server networks,
  ...
  [peer discovery](../discovery/overview) and [content routing](../routing/overview) that help
...
- **Piercing NAT Barriers**: libp2p is equipped with capabilities for [NAT traversal](../nat/overview),
...
- **Message Distribution and Dissemination**: One such pattern libp2p uses is
  [publish/subscribe (pubsub)](../pubsub/overview), which allows a sender (publisher) to send a message
  to multiple recipients (subscribers) without the publisher having to know who the subscribers are.
  libp2p implements pubsub through the use of protocols like [gossipsub](../pubsub/gossipsub), providing
