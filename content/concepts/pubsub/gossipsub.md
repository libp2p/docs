...
of an unsubscribe event, allowing for faster resubscribing.

## The Score Function

The GossipSub protocol uses a scoring function to evaluate the quality of a peer's participation in the
...
Gossipsub v1.1 introduces the concept of extended validators, which allows the application to specify
a more fine-grained message validation process. Extended validators provide more control over how messages
are handled, allowing the application to ignore particular messages without triggering the penalty for
invalid messages.
